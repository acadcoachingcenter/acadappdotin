...
export default function AIQuestionPaperGenerator() {
  ...
  const loadHistory = async () => {
    try {
      const items = await base44.entities.QuestionPaper.list("-created_date", 20);
      setHistory(items);
    } catch (e) {
      console.error(e);
    } finally {
...
- Return ONLY a JSON object matching the schema.`;
            ...
            sections: {
              ...
              items: {
                ...
                properties: {
                  ...
                  type: {
                    type: "string",
                    enum: ["mcq", "short", "long", "fill", "true_false"]
                  },
      ...
      setPaper(result);
      toast({ title: "Question paper generated!" });
    } catch (e) {
      toast({ title: "Generation failed: " + e.message, variant: "destructive" });
    } finally {
      setGenerating(false);
    }
  };
    ...
    try {
      const payload = {
        ...
        topics: (form.topics || "")
          ...
          .map(s => s.trim())
          .filter(Boolean),
        question_distribution: form.question_distribution,
        paper_content: paper,
        instructions: paper.instructions || [],
        status: "generated"
      };
      const rec = await base44.entities.QuestionPaper.create(payload);
      setSavedId(rec.id);
  ...
  const handleCopyText = () => {
    if (!paper) return;
    const lines = [];
    lines.push(paper.paper_title || form.title);
    lines.push(`Subject: ${form.subject}  |  Class: ${form.grade_level}`);
    lines.push(`Max Marks: ${form.total_marks}  |  Duration: ${form.duration_minutes} min`);
    lines.push("");
    (paper.instructions || []).forEach(i => lines.push("• " + i));
    lines.push("");
    (paper.sections || []).forEach(sec => {
      lines.push(`${sec.name}  (${sec.marks_per_question} marks each)`);
      sec.questions.forEach((q, i) => {
        lines.push(`Q${i + 1}. ${q.text}`);
        if (q.options?.length) {
          q.options.forEach((o, j) =>
            lines.push(`   ${String.fromCharCode(65 + j)}) ${o}`)
          );
        }
        if (q.answer) lines.push(`   Ans: ${q.answer}`);
        if (q.answer_hint) lines.push(`   Hint: ${q.answer_hint}`);
      });
      lines.push("");
    });
    navigator.clipboard.writeText(lines.join("\n"));
    toast({ title: "Copied as text" });
  };
  ...
  const loadSavedPaper = (rec) => {
    ...
    setForm({
      ...
      total_marks: rec.total_marks ?? 0,
      duration_minutes: rec.duration_minutes ?? 0,
      topics: (rec.topics || []).join(", "),
      question_distribution: rec.question_distribution || ""
    });
    setSavedId(rec.id);
  ...
  const deletePaper = async (id) => {
    ...
    try {
      await base44.entities.QuestionPaper.delete(id);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="print:hidden flex items-center gap-3">
        ...
        <div>
          <h1 className="text-3xl font-bold text-slate-900">AI Question Paper Generator</h1>
          <p className="text-slate-600">
            Generate complete, syllabus-aligned exam papers with answer keys in seconds.
          </p>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <Card className="print:hidden">
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              <Label htmlFor="dist">Section / Question Distribution</Label>
              <Textarea
                id="dist"
                value={form.question_distribution}
                onChange={e => update("question_distribution", e.target.value)}
                rows={4}
              />
            </div>
          ...
          <CardContent className="p-6">
            {paper ? (
              <>
                ...
                <PaperContent paper={paper} meta={form} />
              </>
            ) : (
        ...
        <CardContent>
          {loadingHistory ? (
            ...
            <p className="text-slate-500 text-sm text-center py-6">
              No papers saved yet. Generate one and click Save.
            </p>
          ) : (
            <div className="grid md:grid-cols-3 gap-3">
              {history.map(rec => (
                <div key={rec.id} className="border rounded-lg p-3 hover:shadow-sm">
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    {rec.total_marks} marks • {rec.duration_minutes} min
                  </p>
                  <div className="flex gap-2 mt-3">
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => loadSavedPaper(rec)}
                    >
