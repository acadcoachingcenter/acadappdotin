...
const SUBJECTS = [
  ...
  "Social Science", "Computer Science", "Economics", "Accountancy", "Business Studies",
  "Others", "Combo"
];
...
export default function CreateCourse() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        console.error("User not found, redirecting...", error);
        navigate(createPageUrl("Welcome"));
      }
    };
    fetchUser();
  ...
  const validateForm = () => {
    const requiredFields = ['title', 'description', 'subject', 'grade_level', 'price', 'duration_weeks', 'max_students'];
    ...
    if (isNaN(maxStudents) || maxStudents <= 0) {
      setError("Please enter a valid maximum number of students");
      return false;
    }
  ...
  const handleSave = async (e) => {
    ...
    try {
      const courseData = {
        ...
        tutor_id: user.id,
        tutor_name: user.full_name || user.email, // Add tutor name
        status: 'draft',
        enrolled_students: 0
      };
      ...
      setError(`Failed to create course: ${error.message || "Unknown error"}`);
    }
    setIsSaving(false);
  };
      ...
      <form onSubmit={handleSave}>
        <Card>
          <CardHeader>
            ...
            <p className="text-slate-600">Fill in the information about the course you want to teach.</p>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <Label htmlFor="grade_level">Grade Level *</Label>
                 <Select value={formData.grade_level} onValueChange={(value) => updateField('grade_level', value)} required>
            ...
            <div className="flex justify-end">
              <Button type="submit" disabled={isSaving} className="bg-[#1565C0] hover:bg-[#1e88e5]">
