...
// time slots across the columns, plus a final "Other Times" column for any
// class scheduled outside those three (the admin scheduler now allows a
// fully custom start/end time, not just the three presets). Used unmodified
// on the Admin, Tutor, and Student Online Classroom pages - each just
// passes in whichever `classes` array it already has (all classes for
// admin, the signed-in user's own classes for tutor/student), so every
// role's timetable stays visually and behaviourally identical without any
// per-role logic living here.
//
...
export default function WeeklyTimetable({ classes, onClassClick }) {
  ...
  function otherClassesFor(day) {
    return classes.filter(
      (c) =>
        c.schedule?.day === day &&
        !TIME_SLOTS.some(
          (slot) => c.schedule?.startTime === slot.startTime && c.schedule?.endTime === slot.endTime
        )
    );
  }

  const renderCell = (cellClasses, { showOwnTime = false } = {}) => (
    <>
          ...
          {cellClasses.map((c) => (
            <div
              ...
              className={`rounded-lg border border-slate-200 bg-slate-50 p-2 ${
                onClassClick ? "cursor-pointer hover:border-slate-400 hover:bg-slate-100" : ""
              }`}
            >
              {showOwnTime && (
                <p className="text-[11px] font-semibold text-slate-600">
                  {formatSlotTime(c.schedule?.startTime)} - {formatSlotTime(c.schedule?.endTime)}
                </p>
              )}
              ...
              {c.tutorName && <p className="mt-0.5 text-[11px] text-slate-400">{c.tutorName}</p>}
              {c.meetUrl && (
                <a
                  href={c.meetUrl}
                  target="_blank"
      ...
      <table className="w-full min-w-[900px] border-collapse text-sm">
        <thead>
          <tr>
            ...
            <th className="border-b border-slate-200 bg-slate-50 p-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">
              Other Times
              <div className="mt-0.5 text-[11px] font-normal normal-case text-slate-400">
                Custom-scheduled classes
              </div>
            </th>
          </tr>
        </thead>
            ...
            <tr key={day} className="align-top">
              ...
              {TIME_SLOTS.map((slot) => (
                <td key={slot.id} className="border-b border-slate-200 p-2 align-top">
                  {renderCell(classesFor(day, slot))}
                </td>
              ))}
              <td className="border-b border-slate-200 p-2 align-top">
                {renderCell(otherClassesFor(day), { showOwnTime: true })}
              </td>
            </tr>
          ))}
        </tbody>
