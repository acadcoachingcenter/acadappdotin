...
export default function AdminBookApprovals() {
  ...
  const loadPurchases = async () => {
    ...
    try {
      const all =
        await apiClient.entities.BookPurchase.list(
          "-created_date",
          200
        );

      setPurchases(Array.isArray(all) ? all : []);
    } catch (error) {
  ...
  const handleApprove = async (purchase) => {
    ...
    try {
      await apiClient.entities.BookPurchase.update(
        purchase.id,
        {
          status: "approved",
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">
          Book Purchase Approvals
        </h1>

        <p className="text-slate-500 text-sm mt-1">
          Verify payment proofs and grant book access
        </p>
      </div>
        ...
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              ...
              <Badge className="bg-slate-200 text-slate-600 ml-1">
                {counts[tab.key]}
              </Badge>
            </button>
          );
