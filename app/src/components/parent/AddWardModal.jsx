...
export default function AddWardModal({ open, onOpenChange, onChildAdded }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const parent = await apiClient.auth.me();
      const emailLower = formData.email.toLowerCase().trim();

      // Check if user already exists
      const existingUsers = await apiClient.entities.User.filter({ email: emailLower });
      ...
      if (existingUsers.length > 0) {
        ...
        await apiClient.entities.User.update(existingStudent.id, {
